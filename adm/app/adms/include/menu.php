<nav class="sidebar">
    <ul class="list-unstyled">
        ...
        $result_menu = "SELECT nivpg.dropdown,
                ...
                pg.id id_pg_menu, pg.nome_paginas, pg.endereco, pg.icone iconpg
                FROM adms_nivacs_pgs nivpg
                INNER JOIN adms_menu men ON men.id=nivpg.adms_menu_id
        ...
        while ($row_menu = mysqli_fetch_assoc($resultado_menu)) {
            ...
            if ($row_menu['dropdown'] == 1) {
                ...
                echo "<li class='$menu_ativado'><a href='" . pg . "/" . $row_menu['endereco'] . "'><i class='" . $row_menu['iconpg'] . "'></i> " . $row_menu['nome_paginas'] . "</a></li>";
                $cont_drop_fech = 1;
            } else {
                if ($cont_drop_fech == 1) {
                    echo "</ul>";
