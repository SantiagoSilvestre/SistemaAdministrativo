<?php 
    ...
    if(!empty($id)) {
        $result_per_resp = "SELECT * FROM sts_pergs_resps WHERE id = '".$id."' LIMIT 1";
       
        $resultado_per_resp = mysqli_query($conn, $result_per_resp);

        if(($resultado_per_resp) && ($resultado_per_resp->num_rows != 0) ){
            $row_per_resp = mysqli_fetch_assoc($resultado_per_resp);
                ...
                <div class="d-flex">
                    ...
                    <div class="content p-1">
                        <div class="list-group-item">
                            ...
                            ?>
                            <form method="POST" action="<?= pg; ?>/processa/proc_sts_edit_per_resp">
                                <input type="hidden" id="id" name="id" value="<?= $row_per_resp['id'] ?>">
                                <div class="form-row">
                                    <div class="form-group col-md-6">
                                        ...
                                        value="<?php if(isset($_SESSION['dados']['pergunta'])) { echo $_SESSION['dados']['pergunta']; } else { echo $row_per_resp['pergunta'];} ?>"
                                        ...
                                        <select name="sts_situacoe_id" id="sts_situacoe_id" class="form-control">
                                            ...
                                            <?php
                                                while($row_sit_per = mysqli_fetch_assoc($resultado_sit_per)) {
                                                    ...
                                                    } else if ( !(isset($_SESSION['dados']['sts_situacoe_id'])) && ($row_sit_per['id'] == $row_per_resp['sts_situacoe_id'])) {
                                                        echo "<option selected value='".$row_sit_per['id']."'>".$row_sit_per['nome']."</option>";
                                                    } else {
                                ...
                                <div class="form-row">
                                    <div class="form-group col-md-12">
                                        ...
                                        <textarea name="resposta" class="form-control" id="resposta"><?php if(isset($_SESSION['dados']['resposta'])) { echo $_SESSION['dados']['resposta']; } else { echo $row_per_resp['resposta'];} ?></textarea>
                                    </div>
                                </div>
                                <p>
                                    <span class="text-danger">* </span>Campo obrigatório
                                </p>
                                <input name="SendStsEditPerResp" id="SendStsEditPerResp"  type="submit" class="btn btn-warning" value="Salvar"></input>
                            </form>
                        </div>
                    <?php          
